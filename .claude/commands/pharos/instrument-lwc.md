...
**Option A — pass the id (only when the Apex method declares the param).** The Apex side (see `pharos:instrument-apex`) resumes a param named `transactionId`. Never add a key the Apex signature doesn't declare — that fails at runtime.
