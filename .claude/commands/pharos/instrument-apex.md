...
Or, to correlate with zero param changes, use platform cache at entry: `Triton.withCache();` (resumes the cached transaction id if present, else starts one; gracefully falls back to a new transaction if cache is unavailable). Pick **one** mechanism per call chain and keep the LWC side consistent (the LWC skill passes a param named `transactionId`).
